...
class PhoneTokenTest(TransactionTestCase):
    ...
    def test_create_otp_for_number(self):
        ...
        otps = self.phone_token.objects.filter(phone_number=phone_number, timestamp__range=(today_min, today_max))
        self.assertTrue(otps.count() <= getattr(settings, 'PHONE_LOGIN_ATTEMPTS', 10), 'message')
